...
def solve_quadratic_equation(a, b, c, direction='right'):
    ...
    if s < 0:
        raise ValueError('Try to calculate the square root of 2 of negative number!')
...
def get_point_from_lengths(point_A, point_B, length_1, length_2, direction='right'):
    ...
    l1 = calculate_distance(point_A, point_B)

    # it is not possible to calculate a point, because the length of l2 and l3 must be greater than l1
    if l1 > length_1 + length_2:
        raise ValueError('The length of l2 and l3 must be greater than l1!')
    ...
    # the point is not in the same x postion
        ...
        return get_point_from_lengths_y_first(xA, yA, xB, yB, length_1, length_2, direction)
...
def get_point(previously_calculated_points, length_array, current_point, cancel_distance = None):
    ...
    # if we do have the number of two of given data elemens, we can choose one direction (left or right)
    if len(data) == 2:
        return get_point_from_lengths(data[0]['point'], data[1]['point'], data[0]['length'], data[1]['length'], 'right')
...
for i in range(len(P_calculated)):
    if len(P_calculated[i]) >= 2:
        print('x = ', '{:.1f}'.format(P_calculated[i][0]), '\ty = ', '{:.1f}'.format(P_calculated[i][1]))
